...
function! s:Cpp_littleLine(title) " {{{
  let pos = (79-strlen(a:title)-4)/2
...
function! s:Cpp_megagroup(name) " {{{
  call s:Cpp_littleLine(a:name)
  call s:Cpp_group(a:name)
endfunction
" }}}

function! s:Cpp_emptyLine(nb) " {{{
  let fo = &fo | set fo-=o
...
function! s:Cpp_addGroup(emptyLine, name, access) " {{{
  if a:emptyLine != 0
...
" }}}
" ==========================================================================
function! s:Cpp_newHeaderFile(name) " {{{
  " reinclusion
...
" Function: Cpp_fileExtension(type) {{{
...
" Function: Cpp_fileName(name0,type) {{{
...
" Function: Cpp_TestInlineFile(filename,type,class) {{{
...
function! s:Cpp_addInlinesInHeader(name)	" Internal use {{{
  normal! G
...
function! s:Cpp_newInlineFile(name)	" Internal use {{{
  normal! G
  call s:Cpp_bigLine( "Inlines")
...
" Function: ReachInlinesZone(type) {{{
...
function! s:WriteInlinePart(name)	" Internal use {{{
  " Class
  call s:Cpp_emptyLine( 1 )
...
" =========================================================================
" Function: AddInlinePart(class,type)	{{{
...
function! s:TabSet(name,i,value) "{{{
...
" Function: Cpp_ReachInlinePart(class)		<external use> {{{
