...
" Avoid reinclusion {{{1
" 
if exists("g:loaded_ui_functions_vim") 
  finish 
endif
let g:loaded_ui_functions_vim = 1
let s:cpo_save=&cpo
set cpo&vim
" }}}1
"------------------------------------------------------------------------
...
function! IF(var,then, else)
  ...
  elseif o =~ 'f\%[te]'           " {{{4
    ...
    echoerr "UI-Fns::IF(): Unkonwn user-interface style (".o.")"
  endif
  " }}}3
endfunction
...
function! SWITCH(var, ...)
  ...
  elseif o =~ 'f\%[te]'           " {{{4
    ...
    echoerr "UI-Fns::SWITCH(): Unkonwn user-interface style (".o.")"
  endif
  " }}}3
endfunction
...
function! CONFIRM(text, ...)
  ...
  if a:0 > 4 " {{{4
    echoerr "UI-Fns::CONFIRM(): too many parameters"
    return 0
  endif
  ...
  elseif o =~ 'f\%[te]'  " {{{4
    ...
    echoerr "UI-Fns::CONFIRM(): Unkonwn user-interface style (".o.")"
  endif
  " }}}3
endfunction
...
function! INPUT(prompt, ...)
  " 1- Check parameters {{{3
  if a:0 > 4 " {{{4
    echoerr "UI-Fns::INPUT(): too many parameters"
    return 0
  endif
  " build the parameters string {{{4
  let i = 1 | let params = ''
  ...
  let o = s:Opt_type()
  if     o =~ 'g\%[ui]'  " {{{4
    exe 'return inputdialog(a:prompt,'.params.')'
  elseif o =~ 't\%[ext]' " {{{4
    exe 'return input(a:prompt,'.params.')'
  elseif o =~ 'f\%[te]'  " {{{4
      exe 'return s:input_fte(a:prompt,'.params.')'
  else               " {{{4
    echoerr "UI-Fns::INPUT(): Unkonwn user-interface style (".o.")"
  endif
  " }}}3
endfunction
...
function! COMBO(prompt, ...)
  " 1- Check parameters {{{3
  if a:0 > 4 " {{{4
    echoerr "UI-Fns::COMBO(): too many parameters"
    return 0
  endif
  " build the parameters string {{{4
  let i = 1
  ...
  elseif o =~ 'f\%[te]'  " {{{4
    ...
    echoerr "UI-Fns::COMBO(): Unkonwn user-interface style (".o.")"
  endif
  " }}}3
endfunction
...
function! CHECK(prompt, ...)
  " 1- Check parameters {{{3
  if a:0 > 4 " {{{4
    echoerr "UI-Fns::CHECK(): too many parameters"
    return 0
  endif
  " build the parameters string {{{4
  let i = 1
  ...
  elseif o =~ 'f\%[te]'  " {{{4
    ...
    echoerr "UI-Fns::CHECK(): Unkonwn user-interface style (".o.")"
  endif
  " }}}3
endfunction
